...
There are basically three requirements for the ranging nodes.
...
Requirement 3 seems to be trivial but is actually the hardest requirement of all three.
Integrating a sensor into the FINken might be a lot of effort but can be done.
Hardware that interacts with the ranging might be substituted by other hardware that does not.
However it is still sensible to pick a ranging method that does not break the current beheaviour of the FINken.

\todo{mehr fokus auf andere copter projekte}
Keeping that in mind  there are some technologies that can be used for ranging.
