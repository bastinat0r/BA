...
The area of operation for the FINken robots is only \SI{3}{m} by \SI{4}{m} so the range measurements need to be sufficiently accurate for those small distances. \todo{provide metric for hypothesis}
...
This means the copter reaches a velocity of over \SI{2}{\metre\per\second} when travelling through the arena at this small angle. \todo{kleine störung-> rel. großer effekt}

\subsection{Environment}
\todo{general indoor flight}
...
Ranging values enable the FINken robots to interact with each other based on their distance i.e. by implementing attraction-repulsion-beheaviour.\todo{more,structure -> annotations}
...
Achtelik et al\cite{Achtelik2009} develop a cheaper vision tracking system.
Nevetheless using an external tracking system is exactly the multi agent systems approach to provide global knowlegde that has been gathered by the tracking system to the agents in contrast to swarm beheaviour that is relying on local information.
...
% \cite{uwb_decawave} claims that an accuracy of \SI{10}{\centi\metre} can be achieved and the nodes are availeble for purchase at reasonable prices. \todo{begründen warum wir das nicht benutzen} \todo{wiederspruch mit den 10cm, more like 50cm}
